...
async function getCrumb(signal: AbortSignal, force = false): Promise<CrumbCache | null> {
...
async function fetchAssetProfile(
  ...
  const attempt = async (forceRefresh: boolean): Promise<AssetProfileResult | null | 'retry'> => {
    const auth = await getCrumb(signal, forceRefresh);
    if (!auth) return null;
    try {
...
async function fetchYahooEarningsTrend(
  ...
  const auth = await getCrumb(signal);
  if (!auth) return null;
