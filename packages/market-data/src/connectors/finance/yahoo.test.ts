...
describe('yahoo finance connector — crumb cancellation', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    __resetYahooCrumbCache();
  });

  it('aborts cookie acquisition when the connector timeout expires', async () => {
    __resetYahooCrumbCache();
    ...
    const connector = createYahooFinanceConnector();
    const response = await connector.getProfile!(
      { instrumentId: 'US:AAPL' },
      { fetchLike: stubFetch({}), timeoutMs: 10 },
    );
