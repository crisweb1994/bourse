...
Analysis V2 的报告模型收敛为五个固定模块：公司质量、行业与竞争、估值与情景、风险清单、市场信号。一次研究始终运行五个模块；四个事实模块并行，风险清单使用前序结果，最后生成综合结论。上线按空库处理，不迁移或兼容旧报告。

用户输入只有四个产品选项：

| 选项 | 值 | 说明 |
|---|---|---|
| 模式 | `QUICK` / `DEEP` | 两种模式都运行五个模块；`QUICK` 一轮少搜索，`DEEP` 允许第二轮交叉核验 |
...
- 重试修改原研究，复用原 Snapshot，只重跑失败/跳过模块及其依赖的风险和综合结论。
