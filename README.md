...
一次研究围绕公司质量、行业与竞争、估值与情景、风险清单、市场信号五个模块展开。模块各自负责不同问题，共享同一份证据快照，避免把同一事实重复写成九段报告。研究可选择 `QUICK` 快速扫描（公司质量、风险清单、市场信号三个模块）或 `DEEP` 深度研究（全部五个模块），并可关注最近 `30D / 90D / 1Y / 3Y` 的价格、新闻和事件变化。

### 财报速读：从公告原文到可核对卡片

Bourse 会定期检测美股、A 股和港股公告，抓取不可变原文，解析章节，抽取逐指标事实，并进行自动一致性检查（逐指标结构化对账为规划中能力，当前按结构化数据源口径展示）。卡片保留来源、版本和状态：
...
| 财报速读 | US SEC / A 股 / 港股公告发现、原文保存、解析、抽取、一致性检查、跨期趋势和版本历史（对账为规划中） |
...
`DEEP` 运行全部五个模块；`QUICK` 只运行三个高信号模块（公司质量、风险清单、市场信号），其余两个标记为“本模式不含”，后续可用 `DEEP` 补全——这是一轮、较少搜索的快速扫描定位。时间窗主要影响行情、新闻和近期事件，财务趋势仍可使用更长历史。系统不提供用户可调的 token、调用次数或费用预算配置。
...
自动一致性检查是概率性拦截，不是数学证明。产品不会承诺“100% 正确”；未通过检查的数字不展示并标注省略数量（逐指标对账状态展示为规划中）。
...
- [x] MetricFact 一致性检查和版本历史（结构化对账:规划中）
