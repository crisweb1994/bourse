...
财报公告默认每 5 分钟检测一次，共识快照每 6 小时更新一次。港股、跨期趋势和 A 股投关记录安装后即可使用，不需要额外功能开关。财报卡只基于可定位的公告原文生成；公告源或模型不可用时不会用无原文的结构化数据替代。EvidencePack 的 web-search fallback 是独立的研究证据链路，不改变财报卡的数据来源要求。系统记录实际 token 和费用，但不维护每日预算预占、结算和回收体系。
